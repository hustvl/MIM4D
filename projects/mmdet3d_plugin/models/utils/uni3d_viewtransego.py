...
class Uni3DViewTransEgo(BaseModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super(Uni3DViewTransEgo, self).__init__()
        if fp16_enabled:
        ...
        if 'None' in sweep_fusion:
            self.sweep_fusion = dict(type="None")
            self.sweep_fusion = self.sweep_fusion.get("type", "None")
        else:
    ...
    def coord_preparing(self, img_metas):
        ...
        ego2img = reference_voxel.new_tensor(np.asarray(ego2img))
        _, num_cam, num_sweep = ego2img.shape[:3]
        ego2img = ego2img.flatten(1, 2)
        ...
        reference_voxel_cam = torch.matmul(
            ego2img.unsqueeze(2), reference_voxel.unsqueeze(1).unsqueeze(-1)
        ).squeeze(-1)
        eps = 1e-5
