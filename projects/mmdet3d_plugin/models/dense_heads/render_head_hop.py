...
class RenderHead(BaseModule):
    ...
    def loss_photometric(self, preds_dict, targets):
        batch_size = len(targets)
        loss_dict = {}
        depth_pred = []
        for bs_idx in range(batch_size):
            i_loss_dict, i_depth_pred = self._loss_photometric(preds_dict[bs_idx], targets[bs_idx])
            for k, v in i_loss_dict.items():
                if k not in loss_dict:
                    loss_dict[k] = []
                loss_dict[k].append(v)
            depth_pred.append(i_depth_pred.unsqueeze(0))
        for k, v in loss_dict.items():
            loss_dict[k] = torch.stack(v, dim=0).mean()
        depth_pred = torch.cat(depth_pred, dim=0)
        return loss_dict, depth_pred.squeeze(0)
    
    @force_fp32(apply_to=("preds_dict", "targets"))
    def _loss_photometric(self, preds_dict, targets):
        depth_pred = preds_dict["depth"]
        depth_pred = depth_pred.view(6, depth_pred.shape[0]//6,-1)
        ...
        loss_dict["rgb_loss"] = rgb_loss * 10


        return loss_dict, depth_pred

    @auto_fp16(apply_to=("pts_feats", "img_feats", "img_depth"))
    def forward(self, uni_feats, rays, img_metas):
        """
        Args:
    ...
    def sample_rays(self, pts, imgs, img_metas):
        ...
        for img_meta in img_metas:
            lidar2img.append(img_meta["keylidar2sweepimg"])
            lidar2cam.append(img_meta["keylidar2sweepcam"])
        lidar2img = np.asarray(lidar2img)
        lidar2cam = np.asarray(lidar2cam)

        staticlidar2img, staticlidar2cam = [], []
        for img_meta in img_metas:
            staticlidar2img.append(img_meta["lidar2img"])
            staticlidar2cam.append(img_meta["lidar2cam"])
        staticlidar2img = np.asarray(staticlidar2img)
        staticlidar2cam = np.asarray(staticlidar2cam)
        ...
        for bs_idx in range(len(pts)):
            ...
            i_staticlidar2img = i_pts.new_tensor(staticlidar2img[bs_idx]).flatten(0, 1)
            ...
            i_staticimg2lidar = torch.inverse(
                i_staticlidar2img
            )  # TODO: Are img2lidar and img2cam consistent after image data augmentation?
            i_staticcam2lidar = torch.inverse(
                i_pts.new_tensor(staticlidar2cam[bs_idx]).flatten(0, 1)
            )
            ...
            for c_idx in range(len(i_pts_mask)):
                ...
                if point_nsample > 0:
                    ...
                    j_sampled_pts = torch.matmul(
                        i_staticimg2lidar[c_idx : c_idx + 1],
                        torch.cat(
                            [
                                j_sampled_pts_cam[..., :2]
                ...
                if len(j_sampled_all_pts) > 0:
                    ...
                    unscaled_ray_o = i_staticcam2lidar[c_idx : c_idx + 1, :3, 3].repeat(
                    ...
                    sampled_pts_cam.append(j_sampled_all_pts_cam)
            ...
            batch_ret.append(
                {
                    ...
                    * self.render_model.scale_factor,
                    "sampled_pts_cam": sampled_pts_cam,
                    "depth_ori": i_sampled_depth / self.render_model.scale_factor,
                    "i_pts_cam":i_pts_cam,
                }
            )
