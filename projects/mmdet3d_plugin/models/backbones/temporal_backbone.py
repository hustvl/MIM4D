...
class BiTemporalPredictor(BaseModule):
    ...
    def __init__(self,
        ...
        self.output_proj = nn.Sequential(
                nn.Conv2d(embed_dims//self.reduction, out_channels, kernel_size=3, padding=1, bias=False),
    ...
    def forward(
        ...
        values_long = torch.stack(values_long, dim=1).reshape(bs*(self.num_adj), C, bev_h*bev_w)
        ...
        if self.with_query:
            ...
            bev_queries_long = values_long.reshape(bev_h*bev_w, bs, self.num_adj, C//self.reduction)[:, :, :1, :].reshape(bev_h*bev_w, bs, C//self.reduction)

        if self.with_sin_embedding:
            values_long = values_long + frame_embeds_long.permute(1, 0, 2).reshape(bs*(self.num_adj), C//self.reduction).unsqueeze(0)
        ...
        bev_embed_long = self.output_proj(bev_embed)
        bev_embed_long = bev_embed_long.view(1, 32, 5, bev_h, bev_w).contiguous()
        return bev_embed_long
    


@BACKBONES.register_module()
class BiTemporalPredictor_longshort(BaseModule):
    """Implements feed-forward networks (FFNs) with identity connection.
    Args:
        embed_dims (int): The feature dimension. Same as
            `MultiheadAttention`. Defaults: 256.
    ...
    def forward(
        ...
        bev_embed = bev_embed.view(1, 32, 5, bev_h, bev_w).contiguous()
        return bev_embed