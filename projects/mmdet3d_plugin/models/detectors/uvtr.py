...
from ..utils import Uni3DViewTrans, Uni3DVoxelPoolDepth
...
class UVTR(MVXTwoStageDetector):
    ...
    def __init__(
        ...
        # if view_trans is not None:
        #     vtrans_type = view_trans.pop('type', 'Uni3DVoxelPoolDepth')
        #     self.view_trans = getattr(view_trans, vtrans_type)(**view_trans)
        if view_trans:
            self.view_trans = Uni3DViewTrans(
                **view_trans
    ...
    def view_transform(self, img_feats, img_depth, pts_feats, img_metas):
        if img_feats is not None:
            uni_feats = self.view_trans(img_feats, img_metas=img_metas, img_depth=img_depth)
        return uni_feats
    ...
    def forward_pts_train(
        ...
        if self.with_depth_head and hasattr(self.view_trans, "loss"):
            losses.update(
                self.view_trans.loss(img_depth, points, img, img_metas)
            )
        return losses
    ...
    def forward_train(
        ...
        uni_feats = self.view_transform(img_feats, img_depth, pts_feat, img_metas)
        losses = dict()
    ...
    def simple_test(self, img_metas, points=None, img=None, rescale=False):
        ...
        img_feats = self.view_transform(img_feats, img_depth, pts_feat, img_metas)
    ...
    def aug_test(self, points, img_metas, imgs=None, rescale=False):
        ...
        img_feats = self.view_transform(img_feats, img_depths, pts_feats, img_metas)
        bbox_list = dict()
