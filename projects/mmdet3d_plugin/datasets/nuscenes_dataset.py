...
class NuScenesSweepDataset(Custom3DDataset):
    ...
    def get_sensor_transforms(self, info):
        # w, x, y, z = info['sensor2ego_rotation']
        # # sweep sensor to sweep ego
        # sensor2ego_rot = torch.Tensor(
        #     pyquaternion.Quaternion(w, x, y, z).rotation_matrix)
        # sensor2ego_tran = torch.Tensor(
        #     info['sensor2ego_translation'])
    ...
    def get_data_info(self, index):
        ...
        if self.modality["use_camera"]:
            ...
            for cam_type, cam_info in info["cams"].items():
                ...
                # sensor(cam) to ego
                w, x, y, z = cam_info['sensor2ego_rotation']
                sensor2ego_rot = pyquaternion.Quaternion(w, x, y, z).rotation_matrix
                sensor2ego_tran = cam_info['sensor2ego_translation']
                cam2ego = np.eye(4)
                cam2ego[:3, :3] = sensor2ego_rot
                cam2ego[:3, -1] = sensor2ego_tran
                camego2global = self.get_sensor_transforms(info)
                cam2global = camego2global.numpy() @ cam2ego
                
                cam2global_rts.append(cam2global)
            ...
            # ego2cam = lidar2cam_rts @ lidar2ego.inverse()
            ego2global = self.get_sensor_transforms(info)
            input_dict.update(
                dict(
            ...
            if "cam_sweep_num" in self.modality:
                ...
                ego2globals_sweeps = []
                cam2global_sweeps_rts = []
                keylidar2sweepimg_sweeps_rts = []
                keylidar2sweepcam_sweeps_rts = []
                ...
                for cam_idx, (cam_type, cam_infos) in enumerate(
                    ...
                    if len(cam_infos) == 0:
                        ...
                        cam2global_sweeps = [lidar2cam_rts[cam_idx] for _ in range(cam_sweep_num)]
                        keylidar2sweepimg_sweeps = [lidar2img_rts[cam_idx] for _ in range(cam_sweep_num)]
                        keylidar2sweepcam_sweeps = [lidar2cam_rts[cam_idx] for _ in range(cam_sweep_num)]
                    else:
                        cam_sweeps = []
                        cam_ids = []
                        cam_time = []
                        ...
                        lidar2img_sweeps = []
                        intrinsics_sweeps = []
                        keylidar2sweepimg_sweeps = []
                        keylidar2sweepcam_sweeps = []
                        for sweep_id, sweep_info in enumerate(
                            ...
                            ego2global = self.get_sensor_transforms(sweep_info)
                            if cam_idx == 0: ## cam_back_left
                                ego2globals_sweeps.append(ego2global)
                            # obtain lidar to image transformation matrix
                            w, x, y, z = sweep_info['sensor2ego_rotation']
                            # sweep sensor to sweep ego
                            sensor2ego_rot = pyquaternion.Quaternion(w, x, y, z).rotation_matrix
                            sensor2ego_tran = sweep_info['sensor2ego_translation']
                            cam2ego = np.eye(4)
                            cam2ego[:3, :3] = sensor2ego_rot
                            cam2ego[:3, -1] = sensor2ego_tran
                            camego2global = self.get_sensor_transforms(info)
                            cam2global = camego2global.numpy() @ cam2ego
                            cam2global_sweeps.append(cam2global)                     
                            ...
                            keylidar2sweepimg_rt = viewpad @ lidar2cam_rt.T
                            keylidar2sweepimg_sweeps.append(keylidar2sweepimg_rt)
                            keylidar2sweepcam_sweeps.append(lidar2cam_rt.T)
                    ...
                    if cam_sweeps[0] == image_paths[cam_idx]:
                        ...
                        cam2global_sweeps_rts.append(cam2global_sweeps[1:cam_sweep_num])
                        cam_sweeps_intrinsics.append(intrinsics_sweeps[1:cam_sweep_num])
                        keylidar2sweepimg_sweeps_rts.append(
                            keylidar2sweepimg_sweeps[1:cam_sweep_num]
                        )
                        keylidar2sweepcam_sweeps_rts.append(
                            keylidar2sweepcam_sweeps[1:cam_sweep_num]
                        )
                    else:
                        raise ValueError
                ...
                # sweep_list = [1,2,4,6]
                sweep_list = [1,2]
                # for sweep_id, sweep_info in enumerate(info['sweeps'][1:cam_sweep_num]):
                len_sweeps = len(info['sweeps'])
                if len_sweeps > 9:
                    for sweep_id in sweep_list:
                        sweep_info = info['sweeps'][sweep_id]
                        ego2global = self.get_sensor_transforms(sweep_info)
                        lidar2egos_sweeps.append(lidar2ego)
                        ego2globals_sweeps2.append(ego2global)
                ego2globals_sweeps = ego2globals_sweeps[1:cam_sweep_num]
                ...
                input_dict.update(
                    dict(
                        ...
                        cam2global_sweeps=cam2global_sweeps_rts,
                        cam_sweeps_intrinsics=cam_sweeps_intrinsics,
                        lidar2egos_sweeps = lidar2egos_sweeps,
                        ego2globals_sweeps = ego2globals_sweeps2,
                        keylidar2sweepimgs=keylidar2sweepimg_sweeps_rts,
                        keylidar2sweepcams=keylidar2sweepcam_sweeps_rts,
                    )
                )
