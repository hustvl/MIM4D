...
class LoadMultiViewMultiSweepImageFromFiles(object):
    ...
    def __call__(self, results):
        ...
        results["keylidar2sweepimg"] = [
            np.stack(
                [
                    results["lidar2img"][_idx],
                    *results["keylidar2sweepimgs"][_idx][:random_num],
                ],
                axis=0,
            )
            for _idx in range(len(results["lidar2img"]))
        ]
        results["keylidar2sweepcam"] = [
            np.stack(
                [
                    results["lidar2cam"][_idx],
                    *results["keylidar2sweepcams"][_idx][:random_num],
                ],
                axis=0,
            )
        ...
        results["cam2global"] = [
            np.stack(
                [
                    results["cam2global"][_idx],
                    *results["cam2global_sweeps"][_idx][:random_num],
                ],
                axis=0,
            )
            for _idx in range(len(results["cam2global"]))
        ]
        ...
        results["lidar2ego"] = [
            np.stack(
                [
                    results["lidar2ego"],
                    *results["lidar2egos_sweeps"][:random_num],
                ],
                axis=0,
            )
        ]
        results["ego2global"] = [
            np.stack(
                [
                    results["ego2global"],
                    *results["ego2globals_sweeps"][:random_num],
                ],
                axis=0,
            )
        ]
        ...
        results.pop("lidar2egos_sweeps")
        results.pop("ego2globals_sweeps")
        results.pop("cam2global_sweeps")
        results.pop("keylidar2sweepimgs")
        results.pop("keylidar2sweepcams")
